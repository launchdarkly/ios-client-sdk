...
{
    return [[EventSource alloc] initWithURL:URL apiKey:apiKey];
}

+ (instancetype)eventSourceWithURL:(NSURL *)URL apiKey:(NSString *)apiKey timeoutInterval:(NSTimeInterval)timeoutInterval
{
    return [[EventSource alloc] initWithURL:URL apiKey:apiKey timeoutInterval:timeoutInterval];
}

- (instancetype)initWithURL:(NSURL *)URL apiKey:(NSString *)apiKey
{
    return [self initWithURL:URL apiKey:apiKey timeoutInterval:ES_DEFAULT_TIMEOUT];
}
...
- (void)_open
{
    ...
    [request addValue:[NSString stringWithFormat:@"api_key %@",self.apiKey] forHTTPHeaderField:@"Authorization"];
    if (self.lastEventID) {
