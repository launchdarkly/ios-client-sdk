...
/**
 ...
 This struct is not stable, and not subject to any backwards compatibility guarantees or semantic versioning. It is
 experimental. Which components make up the identity of an evaluation is the most likely part to be revised, so a
 deduper subclass that reasons about them may need to change with it.
...
/**
 ...
 experimental. Subclassing it to change which evaluations are deduplicated is supported, but the shape it is subclassed
 through, and the components of `EvaluationExposureKey` a subclass reasons about, may change.
