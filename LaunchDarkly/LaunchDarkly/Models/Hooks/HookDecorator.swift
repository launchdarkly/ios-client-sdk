...
/**
 A hook that wraps another hook, forwarding every stage to it. A subclass adds behavior to a hook without changing it,
 and is registered in place of the hook it wraps.

 Every stage forwards to the wrapped hook, so a subclass overrides only the stages it changes, and calls `super` to
 forward the ones it does. The stages it leaves alone still reach the wrapped hook.

 An override that never calls `super` stops forwarding that stage. For the identify and track stages that means a
 decorator swallows something it has no reason to: a `DedupingHook` inside such an override is never told to forget what
 it has reported, and goes on suppressing evaluations across an identify.
 ...
 `DedupingHook` is the decorator the SDK ships: it forwards an evaluation series only when the flag's result is one its
 hook has not just been told about.
...
open class HookDecorator: Hook {
    // Reachable only through `super`. Handing the wrapped hook back out invites code to unwrap a decorator and decide
    // what to do with what it finds, when a decorator is meant to stand in for the hook it wraps.
