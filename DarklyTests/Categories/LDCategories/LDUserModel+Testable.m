...
+(NSDictionary<NSString*, LDUserModel*>*)stubUsersWithKeys:(NSArray<NSString*>*)keys {
    NSMutableDictionary<NSString*, LDUserModel*> *stubbedUsers = [NSMutableDictionary dictionaryWithCapacity:keys.count];
    for (NSString *key in keys) {
        stubbedUsers[key] = [LDUserModel stubWithKey:key];
    }

    return [stubbedUsers copy];
}
...
+(NSArray<NSString*>*)stubUserKeysWithCount:(NSUInteger)count {
    NSMutableArray<NSString*> *userKeys = [NSMutableArray arrayWithCapacity:count];
    for (NSUInteger index = 0; index < count; index++) {
        [userKeys addObject:[[NSUUID UUID] UUIDString]];
    }
    return [userKeys copy];
}
